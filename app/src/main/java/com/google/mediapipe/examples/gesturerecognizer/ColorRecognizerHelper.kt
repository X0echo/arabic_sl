...
class ColorRecognizerHelper(
    ...
    val context: Context,
    val colorRecognizerListener: ColorRecognizerListener? = null
) {
    ...
    fun setupGestureRecognizer() {
        ...
        try {
            ...
            colorRecognizerListener?.onError("Color recognizer failed to initialize. See error logs for details")
            Log.e(TAG, "MP Task Vision failed to load the task with error: ${e.message}")
        } catch (e: RuntimeException) {
            colorRecognizerListener?.onError(
                "Color recognizer failed to initialize. See error logs for details", GPU_ERROR
            )
            Log.e(TAG, "MP Task Vision failed to load the task with error: ${e.message}")
        }
    }
    ...
    fun recognizeImage(image: Bitmap): ResultBundle? {
        ...
        colorRecognizerListener?.onError("Color Recognizer failed to recognize.")
        return null
    }
    ...
    private fun returnLivestreamResult(result: GestureRecognizerResult, input: MPImage) {
        ...
        colorRecognizerListener?.onResults(
            ResultBundle(listOf(result), inferenceTime, input.height, input.width)
        )
    }

    private fun returnLivestreamError(error: RuntimeException) {
        colorRecognizerListener?.onError(error.message ?: "An unknown error has occurred")
    }

    companion object {
        val TAG = "ColorRecognizerHelper ${this.hashCode()}"
        private const val MP_RECOGNIZER_TASK = "colors.task" // Changed to color model
    ...
    interface ColorRecognizerListener {
