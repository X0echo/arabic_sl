...
class NumberRecognizerResultsAdapter :
    RecyclerView.Adapter<NumberRecognizerResultsAdapter.ViewHolder>() {
    ...
    /**
     * Get the current number and confidence score at a specific position.
     * For example, position 0 will give the top result.
     */
    fun getCurrentNumberAndScore(position: Int): Pair<String?, Float?> {
        val category = adapterCategories.getOrNull(position)
        val number = category?.categoryName()
        val score = category?.score()
        return Pair(number, score)
    }
