...
class EducationRecognizerHelper(
    ...
    val context: Context,
    val educationRecognizerListener: EducationRecognizerListener? = null
) {
    ...
    fun setupGestureRecognizer() {
        ...
        try {
            ...
            educationRecognizerListener?.onError("Education recognizer failed to initialize. See error logs for details")
            Log.e(TAG, "MP Task Vision failed to load the task with error: ${e.message}")
        } catch (e: RuntimeException) {
            educationRecognizerListener?.onError(
                "Education recognizer failed to initialize. See error logs for details", GPU_ERROR
            )
            Log.e(TAG, "MP Task Vision failed to load the task with error: ${e.message}")
        }
    }
    ...
    fun recognizeImage(image: Bitmap): ResultBundle? {
        ...
        educationRecognizerListener?.onError("Education Recognizer failed to recognize.")
        return null
    }
    ...
    private fun returnLivestreamResult(result: GestureRecognizerResult, input: MPImage) {
        ...
        educationRecognizerListener?.onResults(
            ResultBundle(listOf(result), inferenceTime, input.height, input.width)
        )
    }

    private fun returnLivestreamError(error: RuntimeException) {
        educationRecognizerListener?.onError(error.message ?: "An unknown error has occurred")
    }

    companion object {
        val TAG = "EducationRecognizerHelper ${this.hashCode()}"
        private const val MP_RECOGNIZER_TASK = "education.task" // Changed to education model
    ...
    interface EducationRecognizerListener {
