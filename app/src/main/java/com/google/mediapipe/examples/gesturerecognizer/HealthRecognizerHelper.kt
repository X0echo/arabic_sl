...
class HealthRecognizerHelper(
    ...
    val context: Context,
    val healthRecognizerListener: HealthRecognizerListener? = null
) {
    ...
    fun setupGestureRecognizer() {
        ...
        try {
            ...
            healthRecognizerListener?.onError("Health recognizer failed to initialize. See error logs for details")
            Log.e(TAG, "MP Task Vision failed to load the task with error: ${e.message}")
        } catch (e: RuntimeException) {
            healthRecognizerListener?.onError(
                "Health recognizer failed to initialize. See error logs for details", GPU_ERROR
            )
            Log.e(TAG, "MP Task Vision failed to load the task with error: ${e.message}")
        }
    }
    ...
    fun recognizeImage(image: Bitmap): ResultBundle? {
        ...
        healthRecognizerListener?.onError("Health Recognizer failed to recognize.")
        return null
    }
    ...
    private fun returnLivestreamResult(result: GestureRecognizerResult, input: MPImage) {
        ...
        healthRecognizerListener?.onResults(
            ResultBundle(listOf(result), inferenceTime, input.height, input.width)
        )
    }

    private fun returnLivestreamError(error: RuntimeException) {
        healthRecognizerListener?.onError(error.message ?: "An unknown error has occurred")
    }

    companion object {
        val TAG = "HealthRecognizerHelper ${this.hashCode()}"
        private const val MP_RECOGNIZER_TASK = "health.task"
    ...
    interface HealthRecognizerListener {
